...
from field_row_detection.geometric import computeTheta, getImgLineUpDown, getLineRphi, lineIntersectImgSides, lineIntersectImgUpDown, lineIntersectWin
...
class imageProc:
    ...
    def trackCropLane(self, mode=None):
        ...
        if self.cropLaneFound:
            ...
            # FIXED
            self.trackingBoxLoc =  self.trackingBoxLoc[self.trackingBoxLoc != 0]
            self.trackingBoxLoc = self.trackingBoxLoc.reshape(-1, 1)
    ...
    def drawGraphics(self):
        ...
        if self.primaryRGBImg.shape[0] > 0:
